...
def main(args):
    ...
    # <<改写>>,小写化所有源/目标语言下的训练集,全部验证集,全部测试集
    def lower_all(lang):
        def lower(prefix):
            input_file = "{}{}".format(
                prefix, ("." + lang) if lang is not None else ""
            )
            output_file = "{}{}".format(
                prefix, (".lower." + lang) if lang is not None else ""
            )
            with open(input_file, 'r', encoding='utf8') as src_f, \
                    open(output_file, 'w', encoding='utf8') as out_f:
                for idx, src_line in enumerate(src_f):
                    out_f.write("{}".format(src_line.lower()))

        if args.trainpref:  # 通过源字典和源训练集路径预处理得到训练集
            lower(args.trainpref)
    ...
    # <<改写>>,小写化所有源/目标语言下的训练集,全部验证集,全部测试集
    lower_all(args.source_lang)
    if target:
        lower_all(args.target_lang)
    # <<改写>>,由小写化的数据集作为新的输入
    args.trainpref = args.trainpref + ".lower"
    args.validpref = ",".join([validpref + '.lower' for k, validpref in enumerate(args.validpref.split(","))])
    args.testpref = ",".join([testpref + '.lower' for k, testpref in enumerate(args.testpref.split(","))])
    ...
    # <<改写>>,删除额外转换的.lower文件
    langlist = [args.source_lang, args.target_lang] if not args.only_source else [args.source_lang]
    for lang in langlist:
        os.remove(args.trainpref + '.' + lang)
        for k, validpref in enumerate(args.validpref.split(",")):
            os.remove(validpref + '.' + lang)
        for k, testpref in enumerate(args.testpref.split(",")):
            os.remove(testpref + '.' + lang)
