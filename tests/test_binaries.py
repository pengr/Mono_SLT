    ...
    def test_raw(self):
        with contextlib.redirect_stdout(StringIO()):
            with tempfile.TemporaryDirectory('test_fconv_raw') as data_dir:
                create_dummy_data(data_dir)
                preprocess_translation_data(data_dir, ['--dataset-impl', 'backup'])
                train_translation_model(data_dir, 'fconv_iwslt_de_en', ['--dataset-impl', 'backup'])
                generate_main(data_dir, ['--dataset-impl', 'backup'])
