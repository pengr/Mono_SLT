...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):  # 通过args,源字典以及源端词嵌入建立编码器,初始化TransformerEncoder类
        return TransformerEncoder(args, src_dict, embed_tokens)  # 返回一个与原始论文一致的Transformer Encoder,每一层TransformerEncoderLayer(Multi-headed attn->dropout->添加残差-> layernorm)
...
　　　# 由*args.decoder_layers*层组成的Transformer decoder,每一层为一个TransformerDecoderLayer类
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):  # 由args初始化一个TransformerDecoderLayer类,注:添加了no_encoder_attn(是否加入编码端输出,即是否存在enc-dec-side self-attn)标记
        return TransformerDecoderLayer(args, no_encoder_attn)  # 返回一个与原始论文一致的Transformer解器层,Multi-headed attn->dropout->添加残差-> layernorm
...
def transformer_wmt_en_de(args):  # 通过-arch选项选择合适的transformer参数
    base_architecture(args)  # 针对args提供进来的参数,已存在的不变,未设置的则按base_architecture设置

# <<改写>>,面向phoenix2014T的slt参数选项
@register_model_architecture("transformer", "transformer_slt")
def transformer_phoenix_slt(args):  # 通过-arch选项选择合适的transformer参数
